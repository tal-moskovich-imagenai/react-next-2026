...
>
> _Source: Boris Cherny, Pragmatic Engineer interview, Sep 2025._
>
...
>
> > _"With an off-distribution stack, the model can still learn it. But you have to show it the ropes and put in the work. We wanted a tech stack which we didn't need to teach: one where Claude Code could build itself."_
>
...
> _"This might sound weird, but the way we build this is we want people to feel the model as raw as possible. We have this belief the model can do much more than products today enable it to do. When you look at a lot of coding products, they get in the way of the model — they add scaffolding, UI elements, and other parts that clutter things, so that the model running in those tools feels like it's hobbling on one foot."_

And the most counterintuitive part:

> _"Every time there's a new model release, we delete a bunch of code. With the 4.0 models, we deleted around half the system prompt because we no longer needed it. We try to put as little code as possible around the model."_
...
This shows up in the patterns below. Each one asks: _what is the minimum Ink needs to do to solve this specific problem?_
