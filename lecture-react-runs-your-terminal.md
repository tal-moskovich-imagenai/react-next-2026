 ...
 **מספר אפליקציו תהריאקט שהם לא בדפדפן******
...
> **Speaker aside — origin story (30 sec, optional if time allows):**
>
> Here's how Claude Code started. Boris Cherny joined Anthropic in September 2024 and built a barebones terminal prototype. The first version couldn't read files or run commands — but it was hooked up to AppleScript so it could tell him what music he was listening to while coding. Neat party trick.
>
> Then he gave it filesystem access. Suddenly it could read a file, follow its imports, read those files, follow their imports — autonomously exploring the entire codebase to answer a question. Boris called it **"product overhang"**: the model could already do this, but no product was built around the capability.
>
> *Source: Boris Cherny, Pragmatic Engineer interview, Sep 2025.*
>
> And one more number: today, around **90% of Claude Code's code is written by Claude Code itself.** The tool builds itself. The renderer you'll see in Block 6 was largely generated by the thing it powers.
...
> **Why TypeScript and React specifically? Primary source:**
>
> The Claude Code team chose this stack deliberately — not just for familiarity, but for a reason specific to AI-assisted development. Boris Cherny called it being **"on distribution"**:
>
> > *"With an off-distribution stack, the model can still learn it. But you have to show it the ropes and put in the work. We wanted a tech stack which we didn't need to teach: one where Claude Code could build itself."*
>
> In AI, "on distribution" means the model has seen so much of a technology during training that it's genuinely fluent in it. TypeScript and React are two of the most well-represented technologies in Claude's training data. Choosing them meant Claude Code could write its own code from day one — and today, ~90% of it does.
>
> The flip side: if you build a terminal tool with an obscure stack, you're fighting the model every step. React + TypeScript + Ink is the path of least resistance, both for you and for any AI assistant you use while building it.
>
> *Source: Boris Cherny, Pragmatic Engineer interview, Sep 2025.*
...
In the browser, `{showPanel && <Panel />}` also causes layout shift — elements below reflow when the panel appears or disappears. But the browser gives you cheap escape hatches: set `display: none` or `visibility: hidden` via CSS and the element stays in the layout flow, taking up its space silently. Problem sidestepped in one line.

In a terminal, **those escape hatches don't exist by default.** Ink's layout is pure document flow — like a webpage with no CSS. When a `<Box>` leaves the tree, everything below jumps up to fill the gap. If you toggle `isPending` rapidly, the input field jumps every time:
...
## Block 4 — Extensions: File Tagging and Drag-and-Drop

> **Speaker note:** Don't live-code this. These are take-home extensions — reference them briefly in Block 4 as "things you can build once you have the primitives," then move on. Both are fully implemented in the repo.

Two questions the chat example immediately invites:

- *"Can I attach a file to my message, like Claude Code's `@filename`?"* → Yes.
- *"Can I drag a file into the terminal from my desktop?"* → Yes, with a small caveat.

---

### Extension 1 — File Tagging (`@`-mention)

The mechanic: detect `@` in `TextInput`, show a file picker, read the selected file, and prepend its content as context in the AI prompt.

**Changes to `TextInput.tsx`:**

```tsx
import { readFileSync, readdirSync } from "fs";

export const TextInput = ({ onSubmit }: { onSubmit: (value: string, attachments: Record<string, string>) => void }) => {
  const [value, setValue] = useState("");
  ...
  useInput((input, key) => {
    ...
    if (atMode) {
      ...
      if (key.downArrow) { setAtIndex(i => Math.min(atFiles.length - 1, i + 1)); return; }
      if (key.return) {
        const file = atFiles[atIndex];
        setAttachments(prev => ({ ...prev, [file]: readFileSync(file, "utf-8") }));
        setValue(prev => prev.replace(/@$/, `@${file} `));
        setAtMode(false);
        return;
      }
      if (key.escape) {
    ...
    // --- Normal input mode ---
    if (key.return) {
      const context = Object.entries(attachments)
    ...
    if (input === "@") {
      setAtFiles(readdirSync(".").filter(f => !f.startsWith(".")));
      setAtIndex(0);
      setAtMode(true);
      setValue(prev => prev + "@");
      return;
    ...
    <Box flexDirection="column">
      ...
      {atMode && (
        <Box flexDirection="column" borderStyle="round" borderColor="yellow" paddingX={1}>
          ...
          {atFiles.slice(Math.max(0, atIndex - 2), atIndex + 6).map((f, i) => {
            const idx = Math.max(0, atIndex - 2) + i;
            return (
              <Text key={f} color={idx === atIndex ? "yellow" : undefined} bold={idx === atIndex}>
                {idx === atIndex ? "❯ " : "  "}{f}
              </Text>
            );
          })}
...
The file content is passed up through `onSubmit` and prepended to the AI prompt as `<file name="…">…</file>` context blocks — the same pattern Claude Code and ChatGPT use for file context.

---

### Extension 2 — Drag and Drop

Two separate problems with two separate solutions.

---

#### Level 1 — File drop from OS (Bracketed Paste Mode)

When you drag a file from Finder/Explorer onto a modern terminal (iTerm2, Ghostty, Kitty, WezTerm, Terminal.app), the terminal pastes the file path into stdin. With **Bracketed Paste Mode** enabled, that paste is wrapped in marker sequences — `\x1b[200~` before, `\x1b[201~` after — so you can tell "this was pasted, not typed."
...
export const useBracketedPaste = (
  ...
  const handler = useCallback(onFileDrop, []);
  ...
  useEffect(() => {
    ...
    const onData = (data: Buffer) => {
      ...
      const match = str.match(/\x1b\[200~([\s\S]*?)\x1b\[201~/);
      if (!match) return;
      const pasted = match[1].trim();
      if (existsSync(pasted)) {
        handler(pasted, readFileSync(pasted, "utf-8"));
      }
    };

    stdin.on("data", onData);
    return () => {
      process.stdout.write("\x1b[?2004l"); // disable on unmount
...
```tsx
useBracketedPaste((filename, content) => {
  setAttachments(prev => ({ ...prev, [filename]: content }));
});
```

Drag a file from your desktop onto the running terminal. It reads the file and attaches it — no `@` typing required.

> **Caveat**: If the user drags a file and BPM is not enabled, or uses a terminal that doesn't support it, they'll see the raw path typed as text. Detect it as a fallback: if `input` matches an existing file path, prompt the user to confirm attachment.

---

#### Level 2 — In-app mouse drag (reorder messages)

Terminals support mouse events via the xterm protocol. Enable it at startup and parse the raw sequences from `useStdin`:
...
export const useMouseDrag = (
  onDrop: (fromRow: number, toRow: number) => void,
) => {
  const { stdin } = useStdin();

  useEffect(() => {
    // xterm mouse tracking: clicks + drag + SGR extended mode
    process.stdout.write("\x1b[?1000h"); // click events
    process.stdout.write("\x1b[?1002h"); // button-motion drag events
    process.stdout.write("\x1b[?1006h"); // SGR mode (handles cols > 223)

    let dragFromRow: number | null = null;
    ...
    const onData = (data: Buffer) => {
      ...
      if (!match) return;
      const [, btn, , row, type] = match;
      const rowNum = parseInt(row) - 1; // terminals are 1-indexed

      if (btn === "0" && type === "M") dragFromRow = rowNum;          // button down
      if (btn === "3" && type === "m" && dragFromRow !== null) {       // button up = drop
        onDrop(dragFromRow, rowNum);
        dragFromRow = null;
      }
    };
...
> **The mapping problem**: the row number from the mouse event is the terminal row (0 = top of screen), not the message index. You need to know which rows each message occupies. Track this with a `rowMap: Map<messageId, {top: number, bottom: number}>` updated on every render, using Ink's `measureElement` if available, or by counting rendered lines per message. This is the hard part — the mouse event protocol is simple, the coordinate→component mapping requires bookkeeping.

**Simpler alternative**: skip true drag and use `Alt+↑` / `Alt+↓` to reorder the selected message. Same end-user power, zero coordinate bookkeeping.

---

### What this shows about the terminal abstraction


| Feature             | Browser                               | Terminal (Ink)                          |
| ------------------- | ------------------------------------- | --------------------------------------- |
| File attach via `@` | Native `<input type="file">`          | `readdirSync` + `useInput` picker       |
| File drop from OS   | `dragover` / `drop` events on DOM     | Bracketed Paste Mode (`\x1b[?2004h`)    |
| In-app mouse drag   | `mousedown` / `mousemove` / `mouseup` | Raw xterm mouse protocol via `useStdin` |


The browser has decades of platform abstractions. The terminal gives you raw bytes — but the bytes are documented, consistent across modern terminals, and perfectly parseable. You're one step closer to the metal, not blocked by it.
...
Before we look at the patterns — a principle that runs through every one of them, and that the Claude Code team has stated explicitly.

Boris Cherny, on the architecture approach:

> *"This might sound weird, but the way we build this is we want people to feel the model as raw as possible. We have this belief the model can do much more than products today enable it to do. When you look at a lot of coding products, they get in the way of the model — they add scaffolding, UI elements, and other parts that clutter things, so that the model running in those tools feels like it's hobbling on one foot."*

And the most counterintuitive part:

> *"Every time there's a new model release, we delete a bunch of code. With the 4.0 models, we deleted around half the system prompt because we no longer needed it. We try to put as little code as possible around the model."*

**The lesson for Ink apps**: the complexity you see in Claude Code's rendering engine isn't there because they love complexity — it's there because they exhausted every simpler option first. The team's rule is: always pick the simplest possible design. Complexity only enters when simplicity genuinely breaks.

This shows up in the patterns below. Each one asks: *what is the minimum Ink needs to do to solve this specific problem?*
...
Ink does **not** run a fixed timer. It is event-driven by React's reconciler. The pipeline on every state commit is:
...
**Step 3: The math — why this breaks under streaming**
...
Per commit:       24,000 OutputEntry objects created
                  24,000 OutputEntry objects from last commit become garbage

At 60 commits/s:  24,000 × 60 = 1,440,000 objects created/second
...
GC behavior:      V8's minor GC (scavenger) runs when the nursery fills.
                  ...
                  every ~10–15ms — right in the middle of your ~16ms commit budget.

Symptom:          GC pause mid-commit → render takes 25ms instead of 16ms
                  → visible stutter in the token stream
```
